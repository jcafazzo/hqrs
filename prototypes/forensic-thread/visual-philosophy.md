...
Scale creates the investigation. One number may fill the field, followed by a small timeline, then a nested system that reveals the same event at several distances. Sparse words function as labels on evidence, never as explanatory paragraphs. Rhythm comes from withholding and disclosure: one condition per beat, one causal assumption revised at a time, each transition executed with master-level restraint.

The invisible craft lies in coherence. Rules, baselines, rings, and repeated markers create a visual grammar that remains stable while the interpretation changes. Nothing overlaps accidentally; nothing drifts from the grid; no element arrives without purpose. The result should appear laboured over with extraordinary care—an austere artifact whose confidence comes from countless small decisions resolved perfectly.
